...
@section('page_title', 'ICT | STAFF | STUDENT INVOICE')
...
@section('content')
    ...
    <div class="card bg-light-info shadow-none position-relative overflow-hidden mb-4">
        <div class="card-body px-4 py-3">
            <div class="row align-items-center">
                <div class="col-9">
                    <h4 class="fw-semibold mb-8">Student Invoice</h4>
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item"><a class="text-muted"
                                    href="{{ route('staff.courses.show', $course->id) }}">{{ Str::limit($course->title, 25) }}</a>
                            </li>
    ...
    <div class="d-flex align-items-center justify-content-between mb-3 no-print">
        <a href="{{ route('staff.courses.show', $course->id) }}"
            ...
            <i class="ti ti-arrow-left fs-4"></i> Back to Course
        </a>
        <button onclick="window.print()" class="btn btn-primary btn-sm d-flex align-items-center gap-1">
            <i class="ti ti-printer fs-4"></i> Print Invoice
        </button>
    </div>
        ...
        <div class="invoice-shell">
            ...
            <div class="inv-meta-grid">
                <div class="inv-meta-cell">
                    <div class="label"><i class="ti ti-user me-1"></i>Student</div>
                    <div class="value">{{ $student->name }}</div>
                </div>
                <div class="inv-meta-cell">
                    <div class="label"><i class="ti ti-book me-1"></i>Course</div>
                    <div class="value text-capitalize">
                        {{ $invoiceItems->pluck('course.title')->filter()->implode(', ') ?: '—' }}
                    </div>
                </div>
                <div class="inv-meta-cell">
                    <div class="label"><i class="ti ti-user-check me-1"></i>Staff</div>
                    <div class="value">{{ $invoice->staff->name ?? 'Staff #' . $invoice->staff_id }}</div>
                </div>
                <div class="inv-meta-cell">
                    <div class="label"><i class="ti ti-credit-card me-1"></i>Payment Option</div>
                    <div class="value text-capitalize">{{ str_replace('_', ' ', $invoice->payment_option) }}</div>
                </div>
                @if ($invoice->paid_at)
                    <div class="inv-meta-cell">
                        <div class="label"><i class="ti ti-calendar-check me-1"></i>Paid At</div>
            ...
            <div class="inv-section">
                <div class="inv-section-title"><i class="ti ti-list-details"></i> Invoice Items</div>
                    ...
                    <div class="inv-totals">
                        ...
                        <div class="inv-total-row">
                            ...
                            <span class="remaining-pill {{ $invoice->remaining_amount == 0 ? 'zero' : 'nonzero' }}">
                                ...
                                @else
                                    <i class="ti ti-alert-circle"></i> ${{ number_format($invoice->remaining_amount, 2) }}
                                @endif
                            </span>
                        </div>
                    </div>
            ...
            <div class="inv-section" style="border-top:1px solid var(--inv-border)">
                <div class="inv-section-title"><i class="ti ti-receipt"></i> Payment History</div>
                    ...
                    <div class="payment-row">
                        ...
                        <div class="flex-grow-1">
                            <div class="d-flex align-items-center gap-2 mb-1">
                                <span class="fw-semibold"
                                    style="font-size:14px;">${{ number_format($payment->amount, 2) }}</span>
                            ...
                            <div class="d-flex align-items-center gap-3 flex-wrap">
                                ...
                                @if ($payment->note)
                                    <span class="text-muted" style="font-size:12px;">
                                        <i class="ti ti-note me-1"></i>{{ $payment->note }}
                                    </span>
                                @endif
                                @if ($payment->paidBy)
                                    <span class="text-muted" style="font-size:12px;">
                                        <i class="ti ti-user me-1"></i>Processed by {{ $payment->paidBy->name }}
                                    </span>
                                @endif
                            </div>
                        </div>
                        <div class="text-end flex-shrink-0">
                            <span class="badge bg-light-success text-success fw-semibold">
                                <i class="ti ti-check me-1"></i>Received
                            </span>
                        </div>
                    </div>
                @empty
                    <div class="text-center py-4 text-muted">
                        <i class="ti ti-inbox fs-5 d-block mb-2"></i>
                ...
                <div class="row align-items-center">
                    <div class="col-md-6">
                        <p class="mb-0 text-muted" style="font-size:12px;">
                            ...
                            ICT Professional Training Center · Generated {{ now()->format('d M Y') }}
                        </p>
                    </div>
    ...
    @else
        <div class="card">
            <div class="card-body text-center py-6">
                ...
                <a href="{{ route('staff.courses.show', $course->id) }}" class="btn btn-outline-primary mt-3">
                    <i class="ti ti-arrow-left me-1"></i> Back to Course
                </a>
            </div>
        </div>
    @endif
