...
@section('content')
    ...
    <div class="card card-body">
        <div class="row align-items-center">
            <div class="col-md-4 col-xl-3">
                <form method="GET" action="{{ route('staff.intern.index') }}" id="search-form">
                    <div class="position-relative">
                        <input type="text" name="search" class="form-control product-search ps-5"
                            placeholder="Search Interns..." value="{{ request('search') }}">
            ...
            <div
                ...
                <a href="javascript:void(0)" id="btn-add-contact" class="btn btn-info d-flex align-items-center"
                    ...
                    <i class="ti ti-users text-white me-1 fs-5"></i> Add Intern
                </a>
            </div>
        </div>
    </div>
    ...
    <div id="view-list" class="card card-body">
                        ...
                        <tr class="search-items">
                            <td>
                                <div class="d-flex align-items-center">
                                    <img src="{{ $intern->image == '' || $intern->image == 'no-img.jpg'
                                        ? ($intern->gender == 'male'
                                    ...
                                    <div class="ms-3">
                                        <div class="user-meta-info">
                                            <h6 class="user-name mb-0">{{ $intern->name }}</h6>
                                            <span class="user-work fs-3 text-muted">{{ $intern->khmer_name ?? '-' }}</span>
                                        </div>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td>
                                <span
                                    class="badge {{ $intern->gender === 'male' ? 'bg-light-primary text-primary' : 'bg-light-danger text-danger' }} rounded-pill">
                                    {{ ucfirst($intern->gender ?? '-') }}
                                </span>
                            </td>
                            <td>{{ $intern->location ?? '-' }}</td>
                            <td>
                                <div class="action-btn">
                                    <a href="javascript:void(0)" class="text-info btn-edit-intern" title="Edit"
                                        data-id="{{ $intern->id }}" data-name="{{ $intern->name }}"
                                        data-khmer-name="{{ $intern->khmer_name }}" data-email="{{ $intern->email }}"
                                        data-phone="{{ $intern->phone }}" data-dob="{{ $intern->dob }}"
                                        data-gender="{{ $intern->gender }}" data-location="{{ $intern->location }}">
                                        <i class="ti ti-edit fs-5"></i>
                                    </a>
                                    <a href="javascript:void(0)" class="text-danger ms-2 btn-delete-intern" title="Delete"
                                        data-id="{{ $intern->id }}" data-name="{{ $intern->name }}">
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="text-center py-4 text-muted">
                                ...
                                No interns found{{ request('search') ? ' for "' . request('search') . '"' : '' }}.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
        ...
        @if ($interns->hasPages())
            {{ $interns->links('frontend.staff.pages.pagination.custom') }}
        @endif
    </div>
    {{-- GRID VIEW --}}
                    ...
                    <div class="card h-100 shadow-sm border-0">
                        <div class="card-body d-flex flex-column align-items-center text-center pt-4">
                            <img src="{{ $intern->image == '' || $intern->image == 'no-img.jpg'
                                ...
                                width="72" height="72">
                            <h6 class="mb-0 fw-semibold">{{ $intern->name }}</h6>
                            @if ($intern->khmer_name)
                                <small class="text-muted">{{ $intern->khmer_name }}</small>
                            @endif
                            <span
                                class="badge {{ $intern->gender === 'male' ? 'bg-light-primary text-primary' : 'bg-light-danger text-danger' }} rounded-pill mt-2">
                                {{ ucfirst($intern->gender ?? '-') }}
                            </span>
                            <hr class="w-100 my-3">
                            <div class="w-100 text-start small text-muted">
                                <div class="mb-1"><i class="ti ti-mail me-1"></i> {{ $intern->email }}</div>
                                <div class="mb-1"><i class="ti ti-phone me-1"></i> {{ $intern->phone ?? '-' }}</div>
                                <div class="mb-1"><i class="ti ti-map-pin me-1"></i> {{ $intern->location ?? '-' }}
                                </div>
                            </div>
                        </div>
                        <div class="card-footer bg-transparent border-0 d-flex justify-content-center gap-2 pb-3">
                            <a href="javascript:void(0)" class="btn btn-sm btn-outline-info btn-edit-intern"
                                data-id="{{ $intern->id }}" data-name="{{ $intern->name }}"
                                data-khmer-name="{{ $intern->khmer_name }}" data-email="{{ $intern->email }}"
                                data-phone="{{ $intern->phone }}" data-dob="{{ $intern->dob }}"
                                data-gender="{{ $intern->gender }}" data-location="{{ $intern->location }}">
                            ...
                            <a href="javascript:void(0)" class="btn btn-sm btn-outline-danger btn-delete-intern"
                                data-id="{{ $intern->id }}" data-name="{{ $intern->name }}">
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header d-flex align-items-center">
                    <h5 class="modal-title">Add Intern</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <div class="add-contact-box">
                        <div class="add-contact-content">
                            <form method="POST" action="{{ route('staff.intern.store') }}" id="addContactModalTitle"
                                    ...
                                    <div class="col-md-6">
                                        <div class="mb-3">
                                            ...
                                            <input type="text" id="c-khmer-name" name="khmer_name"
                                                class="form-control" placeholder="e.g. និស្សិតហាត់ការ"
                                    ...
                                    <div class="col-md-6">
                                        <div class="mb-3">
                                            ...
                                            <input type="email" id="c-email" name="email"
                                                ...
                                                placeholder="e.g. intern@gmail.com" value="{{ old('email') }}">
                                            @error('email')
                                                <span class="text-danger small">{{ $message }}</span>
                                            @enderror
                                        </div>
                                ...
                                <div class="row">
                                    <div class="col-md-12">
                                        <div class="mb-3">
                                            ...
                                            <input type="file" id="c-document" name="document"
                ...
                <div class="modal-footer">
                    <button id="btn-add" class="btn btn-success rounded-pill px-4"
                        ...
                        <i class="ti ti-user-plus me-1"></i> Add Intern
                    </button>
    ...
    <div class="modal fade" id="editContactModal" tabindex="-1" aria-hidden="true" style="display:none;">
        <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form method="POST" id="editInternForm" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="row">
                            <div class="col-md-6">
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="text" name="khmer_name" id="edit-khmer-name" class="form-control"
                                        placeholder="e.g. និស្សិតហាត់ការ">
                                </div>
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="email" name="email" id="edit-email" class="form-control"
                                        placeholder="e.g. intern@gmail.com">
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="mb-3">
                ...
                <div class="modal-footer">
                    <button class="btn btn-success rounded-pill px-4"
                        onclick="document.getElementById('editInternForm').submit();">
    ...
    {{-- DELETE MODAL --}}
    <div class="modal fade" id="deleteInternModal" tabindex="-1" aria-hidden="true" style="display:none;">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header d-flex align-items-center">
                    <h5 class="modal-title text-danger">
                        <i class="ti ti-trash me-2"></i> Delete Intern
                    </h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body text-center py-4">
                    ...
                    <p class="text-muted mb-0">You are about to delete <strong id="delete-intern-name"></strong>. <br>
...
@push('scripts')
    <script>
        ...
        const VIEW_KEY = 'intern_view_preference';
        ...
        // ─── Delete ─────────────────────────────────────────────────────────────────
        document.querySelectorAll('.btn-delete-intern').forEach(function(btn) {
            btn.addEventListener('click', function() {
                const id = this.dataset.id;
                const name = this.dataset.name;
                document.getElementById('delete-intern-name').textContent = name;
                document.getElementById('deleteInternForm').action = `/staff/intern/${id}`;
                new bootstrap.Modal(document.getElementById('deleteInternModal')).show();
            });
        });
        // ─── Edit ───────────────────────────────────────────────────────────────────
        document.querySelectorAll('.btn-edit-intern').forEach(function(btn) {
            btn.addEventListener('click', function() {
                const id = this.dataset.id;
                const form = document.getElementById('editInternForm');
                form.action = `/staff/intern/${id}`;
