...
@push('styles')
    ...
    <style>
        ...
        /* ── Repeater rows (learning points / requirements) ── */
        .repeater-list {
            display: flex;
            flex-direction: column;
            gap: 6px;
        ...
        .repeater-row:hover .repeater-drag {
            opacity: 1;
        }

        .repeater-row .repeater-index {
            width: 22px;
            height: 22px;
            border-radius: 50%;
            background: var(--accent-soft);
...
@section('content')
    ...
    <div class="row">
                ...
                <div class="card-body p-4">
                    ...
                    <form action="{{ route('staff.courses.update', $course) }}" method="POST" enctype="multipart/form-data"
                        ...
                        <div class="form-section">
                            ...
                            {{-- Featured toggle --}}
                            <div class="featured-toggle {{ old('featured', $course->featured) ? 'is-checked' : '' }}"
                                id="featuredToggle">
                                <div class="ft-copy">
            ...
            <div class="row g-3 mb-3">
                ...
                <div class="col-md-3">
                    ...
                    <input type="text" class="form-control" id="end_date" name="end_date"
                        placeholder="Pick a date…"
                        value="{{ old('end_date', optional($course->end_date)->format('Y-m-d')) }}" autocomplete="off">
                ...
                <div class="col-md-3">
                    <div class="form-floating">
                        <input type="number" class="form-control" placeholder="0" name="duration" id="duration"
                            step="0.5" min="0" value="{{ old('duration', $course->duration) }}">
                        <label for="duration">
                ...
                <div class="col-md-3">
                    <div class="form-floating">
                        <input type="number" class="form-control" placeholder="0" name="capacity" id="capacity"
                            min="1" step="1" value="{{ old('capacity', $course->capacity) }}">
                        <label for="capacity">
        ...
        <form method="POST" action="{{ route('staff.course-overview.update', $course) }}" id="courseOverviewForm">
            @csrf
            @method('PUT')
            ...
            @include('frontend.staff.pages.partials.course-repeater-section', [
                ...
                'placeholder' => 'e.g. Basic computer and internet skills',
                'items' => old('requirements', $course->requirements->pluck('content')->toArray()),
            ])

            <div class="d-flex justify-content-end align-items-center gap-3 pt-2">
                <button type="submit" class="btn btn-info btn-submit">
                    <i class="ti ti-device-floppy me-2"></i> Save overview
                </button>
            </div>
        </form>
    </div>
...
@push('scripts')
    ...
    {{-- SortableJS: drag-to-reorder for the learning-point/requirement repeaters --}}
    ...
    <script>
        ...
        /* ── Repeater: add / remove rows (learning points & requirements) ── */
        $(document).on('click', '.repeater-add', function() {
            ...
            const row = $(
                ...
                '<input type="text" class="form-control repeater-input" name="' + name + '" placeholder="' +
                placeholder + '" maxlength="255">' +
                '<button type="button" class="btn btn-sm btn-outline-danger repeater-remove" aria-label="Remove"><i class="ti ti-trash"></i></button>' +
                '</div>'
            );
            list.append(row);
        ...
        $(document).on('click', '.repeater-remove', function() {
            ...
            if (list.children('.repeater-row').length === 0) {
                list.closest('.repeater-section').find('.repeater-add').trigger('click');
            }
        });
