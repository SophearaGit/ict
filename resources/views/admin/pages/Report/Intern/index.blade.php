...
@section('content')
    ...
    <div class="card mb-3 border-0 shadow-sm">
        <div class="card-body py-2 d-flex align-items-center flex-wrap gap-2">
            <ul class="nav nav-pills mb-0 gap-1">
                <li class="nav-item">
                    <a href="{{ route('admin.intern-report.index', request()->except('status')) }}"
                ...
                <li class="nav-item">
                    <a href="{{ route('admin.intern-report.index', array_merge(request()->except('status'), ['status' => 'pending'])) }}"
                ...
                <li class="nav-item">
                    <a href="{{ route('admin.intern-report.index', array_merge(request()->except('status'), ['status' => 'reviewed'])) }}"
    ...
    <div class="card mb-3 border-0 shadow-sm">
        <div class="card-body">
            <form action="{{ route('admin.intern-report.index') }}" method="GET" class="row g-2 align-items-center"
                id="filter_form">
                ...
                <div class="col-6 col-md-2">
                    <select name="sort" class="form-select form-select-sm"
                        ...
                        <option value="intern_asc" {{ request('sort') == 'intern_asc' ? 'selected' : '' }}>Intern A–Z
                        </option>
                        <option value="intern_desc" {{ request('sort') == 'intern_desc' ? 'selected' : '' }}>Intern Z–A
                        </option>
                    </select>
                </div>
                <div class="col-6 col-md-2">
                ...
                <div class="col-12 col-md-1 d-flex gap-2">
                    @if (request()->anyFilled(['search', 'sort', 'year', 'month', 'week']))
                        <a href="{{ route('admin.intern-report.index', request()->only('status')) }}"
                                ...
                                <td>
                                    <div class="d-flex align-items-center gap-2">
                                        <div class="avatar avatar-xs rounded-circle d-flex align-items-center justify-content-center text-white fw-semibold"
                                            style="width:28px;height:28px;background-color: hsl({{ crc32($report->reporter->name ?? 'N') % 360 }}, 55%, 45%); font-size:.75rem;">
                                            {{ strtoupper(substr($report->reporter->name ?? 'N', 0, 1)) }}
                                        </div>
                                ...
                                <td class="text-center">
                                    <div class="d-flex justify-content-center gap-1">
                                        <button class="btn btn-sm btn-icon btn-outline-primary view_report_btn"
                                            ...
                                            data-reporter="{{ $report->reporter->name ?? 'N/A' }}"
                                        ...
                                        @if ($report->status == 'pending')
                                            <button class="btn btn-sm btn-icon btn-outline-success mark_reviewed_btn"
                                                data-url="{{ route('admin.intern-report.review', $report->id) }}"
                                                title="Mark reviewed">
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="border-0">
                                    @include('admin.pages.Report.Intern.partials.empty-state')
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
                                ...
                                <div class="d-flex align-items-center justify-content-between mb-3">
                                    <div class="d-flex align-items-center gap-2">
                                        <div class="avatar avatar-sm rounded-circle d-flex align-items-center justify-content-center text-white fw-semibold"
                                            style="width:32px;height:32px;background-color: hsl({{ crc32($report->reporter->name ?? 'N') % 360 }}, 55%, 45%);">
                                            {{ strtoupper(substr($report->reporter->name ?? 'N', 0, 1)) }}
                                        </div>
                                        <span class="fw-semibold fs-5">{{ $report->reporter->name ?? 'N/A' }}</span>
                                    </div>
                                    @if ($report->status == 'pending')
                                        <span
        ...
        <div class="modal-dialog modal-xl modal-dialog-scrollable">
            <div class="modal-content">
                <div class="modal-header">
                    <div>
                        <h5 class="modal-title mb-0" id="modal_intern_name">Report Detail</h5>
...
@push('scripts')
    <script>
        ...
        $('.view_report_btn').on('click', function() {
            ...
            $('#modal_intern_name').text($(this).data("reporter") + "'s Report");
